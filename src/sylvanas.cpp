...
void synchron(Data *in, Integer prevcnt, Integer threshold, int party){
	...
	// make noisy comparison
	Bit cond_update = (noisy_cnt > threshold);
	if ( !(cond_update.reveal<bool>()) )
		return;
	...
	//reset threshold after update
	reset_threshold(party);
...
int main(int argc, char** argv) {
	...
	Integer threshold = op_recover(th);
	io->flush();

	synchron(data, prev_cnt, threshold, party);
	io->flush();
